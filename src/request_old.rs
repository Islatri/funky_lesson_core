...
pub async fn get_aes_key(client: &Client) -> Result<Vec<u8>> {
    ...
    let key = html
        ...
        .flatten()
        .ok_or_else(|| {
            crate::error::ErrorKind::ParseError("Failed to extract AES key".to_string())
        })?;
...
pub async fn send_login_request(
    ...
    let resp = client.post(login_url).query(&params).send().await?;
...
pub async fn get_aes_key_proxy() -> Result<Vec<u8>> {
    ...
    let resp = build_request("GET", url).await.send().await?;
...
pub async fn get_captcha_proxy() -> Result<(String, String)> {
    ...
    let resp = build_request("POST", url).await.json(&body)?.send().await?;
...
pub async fn send_login_request(
    ...
    let resp = Request::post(login_url).query(params).send().await?;
...
async fn handle_preflight(url: &str) -> Result<()> {
    ...
    request
        .headers()
        .set("Access-Control-Request-Headers", "authorization,batchid")
        .map_err(|e| crate::error::ErrorKind::ParseError(format!("{:?}", e)))?;
