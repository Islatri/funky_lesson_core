...
fn generate_websocket_key() -> String {
    ...
    let encoded = base64.encode_to_string(&key);
    encoded
}
...
impl ICourses {
    ...
    async fn maintain_websocket_heartbeat(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let request = tokio_tungstenite::tungstenite::handshake::client::Request::builder()
            .uri(&ws_url)
            .header("Host", "icourses.jlu.edu.cn")
    ...
    async fn get_favorite(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // let resp_json: serde_json::Value = resp.json().await?;
        let resp_json: serde_json::Value = match resp.text().await {
            Ok(text) => {
                if text.starts_with("<!DOCTYPE html>") {
                    println!("被拦截了");
                    return Err("被拦截了".into());
                }
                serde_json::from_str(&text)?
            }
            Err(err) => {
                println!("❌ 获取收藏课程失败: {}", err);
                return Err("获取收藏课程失败".into());
            }
        };
